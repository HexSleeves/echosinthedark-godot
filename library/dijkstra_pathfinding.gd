...
static func get_coords(map_2d: Dictionary, start_coord: Vector2i, step_length: int) -> Array:
	var neighbor: Array = ConvertCoord.get_diamond_coords(start_coord, step_length)
	var coord: Vector2i
	...
	for i: int in range(0, neighbor.size()):
		coord = neighbor[i]
		if Map2D.is_in_map(coord, map_2d) and _is_valid_distance(map_2d, coord, OBSTACLE):
...
static func set_obstacle_map(map_2d: Dictionary, is_obstacle: Callable, is_obstacle_args: Array) -> void:
...
static func _is_valid_distance(map_2d: Dictionary, coord: Vector2i, max_distance: int) -> bool:
	return (
