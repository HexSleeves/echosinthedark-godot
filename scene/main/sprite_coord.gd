...
func get_sprite_by_coord(main_tag: StringName, coord: Vector2i, z_layer: int) -> Sprite2D:
...
func _add_sprite(sprite: Sprite2D, main_tag: StringName) -> void:
	...
	if _dungeon_sprites.has(hashed_coord):
		...
		if sprites.has(hashed_sprite):
			push_error(
				(
					"Hash collide: %d-%d, %s -> %s."
					% [hashed_coord, hashed_sprite, sprite.name, sprites[hashed_sprite].name]
				)
			)
			return
	else:
...
func _remove_sprite(sprite: Sprite2D, main_tag: StringName) -> void:
	...
	if not sprites.erase(hashed_sprite):
		push_warning("Sprite not hashed: %d-%d, %s." % [hashed_coord, hashed_sprite, sprite.name])
	_set_visibility(coord)
