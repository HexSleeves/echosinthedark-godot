...
func _on_SignalHub_turn_started(sprite: Sprite2D) -> void:
	...
	elif NodeHub.ref_PcAction.alert_duration > 0:
		_approach_pc(_map_2d, sprite, [NodeHub.ref_PcAction.alert_coord])
...
func _is_npc(sprite: Sprite2D) -> bool:
	return sprite.is_in_group(MainTag.ACTOR) and (not sprite.is_in_group(SubTag.PC))


func _approach_pc(map_2d: Dictionary, sprite: Sprite2D, end_point: Array) -> void:
