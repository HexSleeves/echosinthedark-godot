...
func _create_pc(occupied_grids: Dictionary, tagged_sprites: Array) -> Vector2i:
	...
	tagged_sprites.push_back(SpriteFactory.create_actor(SubTag.PC, coord, false))
	return coord
...
func _create_floor(tagged_sprites: Array) -> void:
	for x: int in range(0, DungeonSize.MAX_X):
		for y: int in range(0, DungeonSize.MAX_Y):
			tagged_sprites.push_back(SpriteFactory.create_ground(SubTag.DUNGEON_FLOOR, Vector2i(x, y), false))


func _create_indicator(coord: Vector2i, tagged_sprites: Array) -> void:
	var indicators: Dictionary = {
		SubTag.INDICATOR_TOP: [Vector2i(coord.x, 0), Vector2i(0, -INDICATOR_OFFSET)],
		SubTag.INDICATOR_BOTTOM: [Vector2i(coord.x, DungeonSize.MAX_Y - 1), Vector2i(0, INDICATOR_OFFSET)],
		SubTag.INDICATOR_LEFT: [Vector2i(0, coord.y), Vector2i(-INDICATOR_OFFSET, 0)],
	}
	...
	for i: StringName in indicators:
		...
		tagged_sprites.push_back(CreateSprite.create(MainTag.INDICATOR, i, new_coord, new_offset))


func _create_from_file(path_to_file: String, occupied_grids: Dictionary, tagged_sprites: Array) -> void:
	...
	for i: int in range(0, prefabs.size()):
		...
		packed_prefab = DungeonPrefab.get_prefab(parsed.output_line, _get_edit_tags(EDIT_TAGS))
		...
		_create_from_prefab(packed_prefab, shift_coord, occupied_grids, tagged_sprites)


func _create_from_prefab(
	packed_prefab: PackedPrefab, shift_coord: Vector2i, occupied_grids: Dictionary, tagged_sprites: Array
) -> void:
	...
	for y: int in range(0, packed_prefab.max_y):
		for x: int in range(0, packed_prefab.max_x):
			...
			_create_from_character(
				packed_prefab.prefab[x][y], coord, occupied_grids, random_wall_coords, tagged_sprites
			)
	_create_random_wall(random_wall_coords, occupied_grids, tagged_sprites)


func _create_from_character(
	character: String, coord: Vector2i, occupied_grids: Dictionary, random_wall_coords: Array, tagged_sprites: Array
) -> void:
	match character:
		WALL_CHAR:
			occupied_grids[coord.x][coord.y] = true
			tagged_sprites.push_back(SpriteFactory.create_building(SubTag.WALL, coord, false))
		HOUND_CHAR:
			tagged_sprites.push_back(SpriteFactory.create_actor(SubTag.HOUND, coord, false))
		BULLET_CHAR:
			tagged_sprites.push_back(SpriteFactory.create_trap(SubTag.BULLET, coord, false))
		RANDOM_WALL_CHAR:
...
func _create_random_wall(coords: Array, occupied_grids: Dictionary, tagged_sprites: Array) -> void:
	var max_walls: int = floor(coords.size() * 0.5)

	ArrayHelper.shuffle(coords, NodeHub.ref_RandomNumber)
	coords.resize(max_walls)
	for i: Vector2i in coords:
		occupied_grids[i.x][i.y] = true
		tagged_sprites.push_back(SpriteFactory.create_building(SubTag.WALL, i, false))
